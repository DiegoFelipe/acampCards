...
export default function Home() {
  ...
  const selectCard = (id: number) => {
    setSelectedCard(id);
    setHasSpun(false);
    setTimeout(() => setHasSpun(true), 3000); // espera a animação de 3s terminar
  };
      ...
      className={`${
        isSelected
          ? `fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 ${
              hasSpun ? "scale-110" : "animate-spinSlowZ"
            }`
          : "transition-transform duration-300 hover:scale-105"
      } bg-black border border-green-700 shadow-[0_0_20px_#00ff99] rounded-lg flex flex-col overflow-hidden cursor-pointer`}
      style={{
